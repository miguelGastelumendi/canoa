...
def received_file_get() -> str | Response:
    ...
    try:
        ...
        if request.form.get(js_grid_sec_key) != js_grid_sec_value:
            ...
            ui_texts[ui_msg_exception] = ui_texts["secKeyViolation"]  # TODO
        else:
            task_code += 1  # 4
            no_sep = ui_texts["itemNone"]
            received_files, download_file_name, report_ext = received_files_fetch(no_sep, rec_id)
            if rec_type == dnld_R:
                download_file_name = change_file_ext(download_file_name, report_ext)

            if received_files.count != 1:
                ui_texts[ui_msg_error] = ui_texts["noRecord"]
            elif path.isfile(download_file_name):
                F = send_file(download_file_name)
            else:  # just deleted :-(
                ui_texts[ui_msg_error] = ui_texts["fileNotFound"]
