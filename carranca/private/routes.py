...
def uploadfile():
    ...
    if not is_get:
        ...
        error_code, msg_error, msg_exception, data = upload_file_process(
            logged_user, file_obj, valid_extensions
        )

        if error_code == 0:
            log_msg = add_msg_success(
                "uploadFileSuccess", texts, data.get("file_ticket"), logged_user.email
            )
