...
from ..helpers.py_helper import to_int, clean_text, class_to_dict
...
class select_lists:
    # this is Jinja var style
    schemaList: list[UsualDict] = []
    managerList: list[UsualDict] = []
...
def get_sep_data(task_code: int, edit_mode: SepEditMode, no_manager: NoManager, ui_db_texts: UIDBTexts, sep_id: int, form: SepNew, sep_tmp_name: str) -> Tuple[int, UserSep, Sep, UsualDict, str]:
   ...
   select_list: UsualDict = {}
   def _set_list_values( no_scm: list ):
        ...
        sl = select_lists()
        sl.schemaList = no_scm + Schema.get_schemas(['id', 'name'], 'name').to_list()
        sl.managerList = _get_managers(no_manager)
        sl_dict = class_to_dict(sl)
        return sl_dict
   ...
   if sep_row is None:
        ...
        pass
         ...
         select_list= _set_list_values(no_scm)
         return return_task_code, None, sep_row, select_list, ui_db_texts["sepNewTmpName"] # = 'Novo SEP'
   else:
   ...
   return return_task_code, usr_sep, sep_row, select_list, sep_usr_row.fullname
