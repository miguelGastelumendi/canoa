...
def register(cargo: Cargo, file_data: object | str) -> Cargo:
    ...
    try:
        ...
        cargo.storage.working_file_name = f"{file_ticket}_{cargo.storage.received_file_name}"
        working_file_full_name = cargo.storage.working_file_full_name()

        file_size = 0
        file_crc32 = crc32(b'')  # Initialize CRC32 checksum
        # create the uploaded_file, from file_obj
        with open(working_file_full_name, "wb") as file:
            task_code += 1 # +1
            file_data = file_obj.read()
            task_code += 1 # +2
            file_crc32 = crc32(file_data, file_crc32)
            task_code += 1 # +3
        ...
        user_record_to_update = UserDataFiles(
            ...
            , file_name = cargo.storage.received_file_name
            , original_name = cargo.storage.received_original_name
            , ticket = file_ticket
        ...
        if file_saved and not file_registered:
            os.remove(working_file_full_name)
