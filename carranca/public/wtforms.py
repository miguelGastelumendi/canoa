...
max_name = LenValidate(
    min(g.app_config.len_val_for_uname.min, g.app_config.len_val_for_email.min),
    max(g.app_config.len_val_for_uname.max, g.app_config.len_val_for_email.max),
)
...
class RegisterForm(FlaskForm):
    username = StringField(
        "",
        validators=[InputRequired(), Length(**g.app_config.len_val_for_uname.wtf_val())],
    )
    email = StringField(
        "",
        validators=[
            InputRequired(),
            Email(),
            Length(**g.app_config.len_val_for_email.wtf_val()),
        ],
    )
    password = PasswordField(
        "",
